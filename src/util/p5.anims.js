;(function (global, factory) {
  ...
  function getEndPoint(vertex) {
    if (vertex instanceof Vertex) return new Vertex(vertex.x, vertex.y)
    else return new Vertex(vertex.x4, vertex.y4)
  }
  ...
  function partialBezierCurve(bezierCurve, t0, t1) {
    ...
    const qxa =
      bezierCurve.x1 * u0 * u0 +
      bezierCurve.x2 * 2 * t0 * u0 +
      bezierCurve.x3 * t0 * t0
    const qxb =
      bezierCurve.x1 * u1 * u1 +
      bezierCurve.x2 * 2 * t1 * u1 +
      bezierCurve.x3 * t1 * t1
    const qxc =
      bezierCurve.x2 * u0 * u0 +
      bezierCurve.x3 * 2 * t0 * u0 +
      bezierCurve.x4 * t0 * t0
    const qxd =
      bezierCurve.x2 * u1 * u1 +
      bezierCurve.x3 * 2 * t1 * u1 +
      bezierCurve.x4 * t1 * t1

    const qya =
      bezierCurve.y1 * u0 * u0 +
      bezierCurve.y2 * 2 * t0 * u0 +
      bezierCurve.y3 * t0 * t0
    const qyb =
      bezierCurve.y1 * u1 * u1 +
      bezierCurve.y2 * 2 * t1 * u1 +
      bezierCurve.y3 * t1 * t1
    const qyc =
      bezierCurve.y2 * u0 * u0 +
      bezierCurve.y3 * 2 * t0 * u0 +
      bezierCurve.y4 * t0 * t0
    const qyd =
      bezierCurve.y2 * u1 * u1 +
      bezierCurve.y3 * 2 * t1 * u1 +
      bezierCurve.y4 * t1 * t1
  ...
  class Shape {
    ...
    update() {
      ...
      for (let i = 0; i < currentCurveNo; i++) {
        ...
        if (v instanceof Vertex) this.p5obj.vertex(v.x, v.y)
        else this.p5obj.bezierVertex(v.x2, v.y2, v.x3, v.y3, v.x4, v.y4)
      ...
      if (currentCurveNo < numCurves) {
        ...
        if (v instanceof Vertex) {
          ...
          const partial = partialBezierCurve(
            currentCurve,
            0,
            currentCurveProgress
          )
          this.p5obj.bezierVertex(
            partial.x2,
            partial.y2,
            partial.x3,
            partial.y3,
  ...
  class AnimShapes {
    ...
    getOrCreateShapeInstance_(id, getShapeVertices, duration) {
      if (this.checkDuplicateId_(id)) {
        throw new Error(
          'AnimS id must not be duplicate in the same p5.js frame.'
        )
      }
    ...
    arc(id, duration, x, y, w, h, start, stop) {
      ...
      if (start >= stop || stop - start < epsilon) return
        ...
        () => {
          ...
          for (const v of vertices) {
            if (v.length == 2) {
              ...
              shapeVertices.push(
                new BezierVertex(v[0], v[1], v[2], v[3], v[4], v[5])
              )
            } else {
              throw new Error(
                'Only simple vertices (x, y) or Bezier vertices' +
